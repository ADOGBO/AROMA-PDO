...
def add_args(parser):
    ...
    parser.add_argument("--num_sample", type=int, default=5,help="number of samples of trajectories")
    ...
    parser.add_argument("--mass_weight", type=float, default=1.,help="the mass weight in metric")
    parser.add_argument("--grad_weight", type=float, default=10.,help="the gradient weight in metric")
    parser.add_argument("--energy_weight", type=float, default=0.1,help="the energy weight in metric")
...
if __name__ == "__main__":
    ...
    ckpt_path = checkpoint_dir / "checkpoint_metrics.pt"
    tmp_path= checkpoint_dir / "checkpoint_metrics.tmp"
    last_model_path=checkpoint_dir / "checkpoint_metrics_lastModel.pt"

    train_losses_last_path=checkpoint_dir / "losses_train_metrics_l.npy"
    train_losses_mean_path=checkpoint_dir / "losses_train_metrics_m.npy"
    train_losses_attn_path=checkpoint_dir / "losses_train_metrics_a.npy"

    test_losses_last_path=checkpoint_dir / "losses_test_metrics_l.npy"
    test_losses_mean_path=checkpoint_dir / "losses_test_metrics_m.npy"
    test_losses_attn_path=checkpoint_dir / "losses_test_metrics_a.npy"
    ...
    state_DiT=torch.load(DiT_path,weights_only=False)
    ...
    for step in range(state.epoch,cfg.max_iterations): 
        ...
        for step_train_loader,batch in enumerate(train_loader):
            ...
            pertubation=cfg.pertu*torch.randn( cfg.num_sample,*u.shape ) #shape (E, b, T, N, d)
            ...
            with autocast(enabled=False): # mixed precision
                with torch.no_grad():
                    ...
                    sammple_amenaged=rearrange(sample,"(E b T) N d -> (E b) T N d",E=cfg.num_sample, b=batch_size_tr)

                    y=[]  
                    for t in range(time_step - 1 ):
                        
                        sample_prev=sammple_amenaged[:,t,:,:]  #sample_prev shape (Eb,N,d)
                    ...
                    out=rearrange(out, "(E b T) M h ->E b T M h",E=cfg.num_sample,b=batch_size_tr) # shape (E, b, (T-1),N,d)
            ...
            print(f"++++++++++step={step:<10d} ---loss_mean={loss_m.detach().cpu().numpy():.4f} ---loss_last={loss_l.detach().cpu().numpy():.4f} ---loss_attn={loss_a.detach().cpu().numpy():.4f}  LRE = {state.scheduler_attn.get_last_lr()[0]:.6f}")
        ...
        if step % cfg.log_interval == 0:
            ...
            with torch.no_grad():
                ...
                with autocast(enabled=False): # mixed precision
                    ...
                    #samples=torch.cat(samples,dim=0)
                    sammple_amenaged=rearrange(sample,"(E b T) N d -> (E b) T N d",E=cfg.num_sample,b=batch_size_test)
                    ...
                    out=state_enco_deco.model_deco(y,x_out) # shape (Eb(T-1),N,d)
                    out=rearrange(out, "(E b T) M h ->E b T M h",E=cfg.num_sample,b=batch_size_test) # shape (E, b, (T-1),N,d)
